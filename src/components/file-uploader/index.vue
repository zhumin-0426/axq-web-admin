 ...
 * @LastEditTime: 2022-05-04 20:24:33
...
<template>
  <div class="file-uploader">
    ...
    <div class="u-list">
        <div class="l-item" :key="key"
          v-for="(item, key) in fileList">
          <img class="file-img" :src="item.oss.url" alt="" v-if="viewEnableExt.includes(item.info.name.getExt())"
            @click="showPopup(item)">
          <template v-else>
            <div class="file-icon">
              <i class="iconfont" :class="item.info.name | fileIconfontFormat" @click="showPopup(item)"></i>
            </div>
          </template>
          <p>{{ item.info.name }}</p>
...
import { OssLib } from '@/libs/js/oss';
