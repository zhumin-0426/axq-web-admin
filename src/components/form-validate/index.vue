  ...
  data () {
    return {
      step: {
        ...
        fn: {
          _check: () => {
            return formToolkit.validate(this.form, { require: this.stepConfig.list[this.step.active]._validateField, forceValidField: Array.isArray(this.forceValidField) ? this.forceValidField : this.forceValidField() });
          },
          stepChange: (n) => {
            if (n < 0) {
