<template>
  <div class="list-page">
    <div class="p-toolbar" v-if="!hiddenToolbar">
      <div class="t-left">
        <div class="l-query">
        ...
        <div class="r-item">
          <el-button-group>
            ...
            <el-button
              size="small"
              @click.native="
                $refs.popupConfirmExcelExport.open({
                  triggerClose: popup.excelExport.fn.closeHandler,
                })
              "
              v-if="
                popup.excelExport.attr.enable &&
                !(Array.isArray(hiddenAction) ? hiddenAction : hiddenAction()).includes(
                  'excel'
                )
              "
              >导出</el-button
...
export default {
            ...
            closeHandler: (ok, error) => {
              ...
              this.request
                .do(
                  ...
                  domain,
                  action,
                  this.loadExcelConfig ? this.loadExcelConfig() : { params: {}, data: {} }
                )
                .then((res) => {
  ...
  computed: {
    excelExportActive() {
      for (let i = 0; i < this.popup.excelExport.attr.options.length; i++) {
        if (
          this.popup.excelExport.attr.options[i].value ===
          this.popup.excelExport.attr.optionsActive
        ) {
  ...
  methods: {
    _buildExcel() {
      if (
        this.config.entity &&
        this.config.allowExportExcel &&
        entitys[this.config.entity] &&
        entitys[this.config.entity]._excel
      ) {
        this.popup.excelExport.attr.enable = true;
        this.popup.excelExport.attr.options = this.excelFilter
          ? this.excelFilter(entitys[this.config.entity]._excel)
          : entitys[this.config.entity]._excel;
      }
    },
    _buildApi() {
    ...
    getPage() {
      this._buildApi().then((res) => {
        ...
        if (status) {
          this.tableData =
            this.config.isRaw || this.config.req
              ? data
              : data.map((i) =>
                  Object.assign(new entitys[this.config.entity](i), { _raw: i })
                );
    ...
    exportExcel() {
      ...
      if (eleFix) {
        wb = XLSX.utils.table_to_book(
          document.querySelector(".el-table").removeChild(eleFix)
        );
