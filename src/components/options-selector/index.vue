 ...
 * @LastEditTime: 2021-12-10 08:42:05
...
export default {
  ...
  data () {
    return {
      ...
      props: {
        lazy: true,
        checkStrictly: this.config.checkStrictly ? true : false,
  ...
  methods: {
    init () {
      if (this.mode === 1) {
        ...
        this.options = (this.config.options ? (typeof this.config.options === 'function' ? this.config.options() : this.config.options) : collection[this.config.entity]._options[this.config.type]).filter(i => {
